...
def index(request):
    ...
    contryNames,countsVal,logVals,overallCount,dataForMapGraph,maxVal=getBarData(confirmedGlobal,uniqueCountryNames) #Dado mapa 
    dataForheatMap,dateCat=getHeatMapData(confirmedGlobal,contryNames) #Dado mapa 
...
def getLinebarGroupData(confirmedGlobal):
    ...
    top10 = confirmedGlobal[['Country/Region', confirmedGlobal.columns[-2]]].groupby('Country/Region').sum().sort_values(by=str(confirmedGlobal.columns[-2]), ascending=False)
    top10 = top10.iloc[0:10,:] # exibindo a evolução da covid nos atuais 10 país com mais casos
